...
def active_listings(session: Session, city: str):
    """
    Calculate the number of active listings in the most recent quarter.
    """
...
def active_listings_delta(session: Session, city: str):
    """
    Calculate the change in the number of active listings from four quarters prior.
    """
...
def active_hosts(session: Session, city: str):
    """
    Calculate the number of unique hosts with an active listing in the most recent quarter.
    """
...
def median_price(session: Session, city: str):
    """
    Calculate the median listing price for active listings in the most recent quarter.
    """
    ...
    prices = [p[0] for p in prices]  # Extract prices from tuples
    return sorted(prices)[len(prices) // 2] if prices else None


def median_review_score(session: Session, city: str):
    """
    Calculate the median review score for active listings in the most recent quarter.
    """
    ...
    scores = [s[0] for s in scores]  # Extract scores from tuples
    return sorted(scores)[len(scores) // 2] if scores else None


def median_price_delta(session: Session, city: str):
    """
    Calculate the change in the median listing price from four quarters prior.
    """
    ...
    previous_prices = [p[0] for p in previous_prices]  # Extract prices from tuples
...
def median_review_score_delta(session: Session, city: str):
    """
    Calculate the change in the median review score from four quarters prior.
    """
    ...
    previous_scores = [s[0] for s in previous_scores]  # Extract scores from tuples
